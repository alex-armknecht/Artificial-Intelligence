...
class InferenceModule:
    ...
    ######################################
    # little dots on ur map 
    # Think about this: 
    # each point on the map has an integer associated with it about how many particles are in it 
    # we have a list of legal positions 
    # for every legal positions if you make an array where it indexes to the appropriate square.

    def initializeUniformly(self, gameState):
        "Sets the belief state to a uniform prior belief over all positions."
        #uniform value = 1/# of legal positions
        #loop through each position and assign uniform value

    def observe(self, observation, gameState):
        "Updates beliefs based on the given distance observation and gameState."
        #updates self.beliefs for observation and gameState 
        # self.beliefs.update
        #if (noisyDistance == None):
            # self.beliefs[self.getJailPosition()] = 1.0
            #self.beliefs.update 
            # else
        #somewhere we need to put: if prob = 0, resample(or initializeUniformly?)
         #edge case of if # of ghosts decreases, resample and place ghost position in jail
        pass

    def elapseTime(self, gameState):
        "Updates beliefs for a time step elapsing from a gameState."
        #sets gameState to the most likely belief state then initializeUniformly for self.beliefs?
...
class ExactInference(InferenceModule):
    ...
    def elapseTime(self, gameState):
        ...
        *** GORY DETAIL AHEAD ***
        ...
        for cur_pos, prob in self.beliefs.items(): 
            new_pos_dist = self.getPositionDistribution(self.setGhostPosition(gameState, cur_pos))
            for new_pos, prob2 in new_pos_dist.items() : 
                new_beliefs[new_pos] += prob2*prob
...
class ParticleFilter(InferenceModule):
    ...
    def initializeUniformly(self, gameState):
        ...
        "*** YOUR CODE HERE ***"
        self.particles = [] 
        start_value = self.numParticles / len(self.legalPositions)
        for pos in self.legalPositions:
             self.particles.extend([pos]*start_value)
    ...
    def observe(self, observation, gameState):
        ...
        part_positions = util.Counter()
        if (noisyDistance == None):
            part_positions[self.getJailPosition()] = 1.0
        else:
            for p in self.particles:
                true_distance = util.manhattanDistance(p, pacmanPosition)
                part_positions[p] +=  emissionModel[true_distance] 

        if part_positions.totalCount() == 0 : 
            self.initializeUniformly(gameState)

        else: 
            part_positions.normalize() 
            partie = []
            for p in self.particles :
                partie.append(util.sample(part_positions))
            self.particles = partie
        self.beliefs = part_positions
    ...
    def elapseTime(self, gameState):
        ...
        for p in self.particles:
            ...
            partics.append(util.sample(new_pos_dist))
        self.particles = partics
